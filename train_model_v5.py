"""
Tradeora Model v5 Training Script
===================================
Key improvements over v4:
  1. Integrates EGX Foreign & Institutional Investor Flow Features (daily_investor_flows)
  2. Adds 4 new Investor Flow Features:
     - foreigners_net_norm: Foreigners Net EGP normalized by market volume
     - foreign_inst_net_norm: Foreign Institutions Net EGP normalized
     - egyptian_inst_net_norm: Egyptian Institutions Net EGP normalized
     - flow_trend_3d: 3-day Foreign Net Flow streak (+1 = Buy streak, -1 = Sell streak)
  3. 29 features total (Technical, Engine Confluences, Microstructure, Investor Flows)
  4. TimeSeriesSplit cross-validation with Calibrated probability predictions
"""
...
MODEL_OUT  = 'models/model_1d_v5.pkl'
SCALER_OUT = 'models/scaler_1d_v5.pkl'
META_OUT   = 'models/model_v5_metadata.json'
...
FEATURE_NAMES = [
    ...
    'foreigners_net_norm',    # Foreigners Net EGP / Total Market Volume
    'foreign_inst_net_norm',  # Foreign Institutions Net / Volume
    'egyptian_inst_net_norm', # Egyptian Institutions Net / Volume
    'flow_trend_3d',          # 3-day Foreign Net Buying/Selling streak
]

print("=" * 65)
print("  TRADEORA MODEL v5 TRAINING  (29 Features + Investor Flows)")
print("=" * 65)

# ── STEP 1: Load all market data & investor flows ─────────────────────────────
print("\n📥 Loading market prices & investor flows...", flush=True)
...
# Fetch Daily Investor Flows map by trade_date
flow_res = sb.table('daily_investor_flows').select('*').execute().data or []
flow_map = {}
for fl in flow_res:
    flow_map[fl['trade_date']] = fl

print(f"   {len(flow_res):,} investor flow rows mapped", flush=True)
...
print("\n⚙️ Extracting 29 features vectorized per stock...", flush=True)
...
for cid, raw_rows in by_co.items():
    ...
    pb        = float(co_fund.get('pb_ratio') or 1.5)
    ...
    # Compute 3-day flow trend streak
    for i_idx in range(2, N):
        ...
        if fn0 > 0 and fn1 > 0 and fn2 > 0:
            flow_trend_3d_vec[i_idx] = 1.0
        elif fn0 < 0 and fn1 < 0 and fn2 < 0:
            flow_trend_3d_vec[i_idx] = -1.0

    # Stack features: shape (N, 29)
...
print("\n🤖 Training XGBoost Model v5 (Calibrated)...", flush=True)
...
print(f"\n📈 Out-Of-Fold Validation Results (v5):")
...
meta = {
    ...
    'description': 'Tradeora v5 model trained with 29 features including EGX Foreign & Institutional Investor Flows.'
}

with open(META_OUT, 'w', encoding='utf-8') as f:
    json.dump(meta, f, indent=2, ensure_ascii=False)

print(f"\n🎉 SUCCESS! Saved v5 Model to {MODEL_OUT}")
