...
export interface SubscriptionEntitlements {
  ...
  aiRecommendationsPerDay: number; // -1 for unlimited
  priceAlertsLimit: number; // -1 for unlimited
  historicalDataYears: number; // -1 for full
...
export interface SubscriptionPlan {
  ...
  nameAr: string;
  nameEn: string;
  monthlyPriceEgp: string; // Decimal string representation
  annualPriceEgp: string;  // 20% discount applied
...
export class VATCalculator {
  private static readonly VAT_RATE = new Decimal('0.14'); // 14% Egypt VAT
